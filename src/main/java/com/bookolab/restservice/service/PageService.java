...
public class PageService {
    ...
    */
    ...
    public PageResponseDto getFlatPage(Long id) {
        ...
        FlatLinkedWordDto firstWordDto = first != null ? new FlatLinkedWordDto(first.getId(), first.getLocalId(), first.getContent(), firstNextId, firstPreviousWordId, firstAuthorName) : null;
        FlatLinkedWordDto lastWordDto = last != null ? new FlatLinkedWordDto(last.getId(), last.getLocalId(), last.getContent(), lastNextId, lastPreviousWordId, lastAuthorName) : null;

        // 🚀 Add individual checks for the flat layout boundaries
        if (firstWordDto != null) {
            firstWordDto.setLikeCount(reactionRepository.countByWordIdAndReactionType(first.getId(), ReactionType.LIKE));
            firstWordDto.setDislikeCount(reactionRepository.countByWordIdAndReactionType(first.getId(), ReactionType.DISLIKE));
        }
        if (lastWordDto != null) {
            lastWordDto.setLikeCount(reactionRepository.countByWordIdAndReactionType(last.getId(), ReactionType.LIKE));
            lastWordDto.setDislikeCount(reactionRepository.countByWordIdAndReactionType(last.getId(), ReactionType.DISLIKE));
        }
    ...
    */
    ...
    public BoundedPageResponse getBoundedPage(Long id, String username) {
        ...
        if (currentEntity != null && lastEntity != null) {
            ...
            if (!allDtosOnPage.isEmpty()) {
                ...
                for (WordNodeDto dto : allDtosOnPage) {
                    long likes = rawCounts.stream()
                            .filter(m -> m.get("wordId").equals(dto.getId()) && m.get("type") == ReactionType.LIKE)
                            .mapToLong(m -> (Long) m.get("cnt")).findFirst().orElse(0L);

                    long dislikes = rawCounts.stream()
                            .filter(m -> m.get("wordId").equals(dto.getId()) && m.get("type") == ReactionType.DISLIKE)
                            .mapToLong(m -> (Long) m.get("cnt")).findFirst().orElse(0L);
            ...
            flatLastWord = new FlatLinkedWordDto(lastEntity.getId(), lastEntity.getLocalId(), lastEntity.getContent(), nextId, previousWordId, lastWordAuthorName);

            // Enforce counts on the standalone flatLastWord DTO as well
            if (flatLastWord != null) {
                flatLastWord.setLikeCount(reactionRepository.countByWordIdAndReactionType(lastEntity.getId(), ReactionType.LIKE));
                flatLastWord.setDislikeCount(reactionRepository.countByWordIdAndReactionType(lastEntity.getId(), ReactionType.DISLIKE));
            }
        }
